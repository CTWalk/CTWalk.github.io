...
const probe = (checkpointId) => {
  ...
  const brokenImages = allImgs.filter(i => !i.complete || i.naturalWidth === 0).map(i => ({ src: (i.currentSrc || i.src).slice(-70), complete: i.complete, nw: i.naturalWidth }));
  const base = {
    ...
    broken_images: brokenImages, viewport: { vw, vh }
  };
  ...
  if (presentation === 'mobile-fallback') {
    ...
    const visibleScenes = scenes.filter(vis).map(s => s.dataset.scene);
    const st = window.__portfolioTest.getState();
    const titleFs = title ? parseFloat(getComputedStyle(title).fontSize) : null;
    return { ...base,
      mobile: {
        ...
        avatar_natural: avatar ? [avatar.naturalWidth, avatar.naturalHeight] : null,
        avatar_rendered: avatar ? [Math.round(avatar.getBoundingClientRect().width), Math.round(avatar.getBoundingClientRect().height)] : null,
        avatar_centered: avatar ? Math.abs((avatar.getBoundingClientRect().left + avatar.getBoundingClientRect().right) / 2 - vw / 2) < 6 : null,
        cta_text: cta?.textContent?.trim() ?? null, cta_href: cta?.getAttribute('href') ?? null,
        cta_below_avatar: (avatar && cta) ? cta.getBoundingClientRect().top >= avatar.getBoundingClientRect().bottom - 2 : null,
        guidance_text: msg?.textContent?.trim() ?? null, guidance_lines: lines(msg),
        experience_hidden: experience ? (experience.hidden || getComputedStyle(experience).display === 'none' || experience.getAttribute('aria-hidden') === 'true') : null,
        visible_desktop_scenes: visibleScenes,
        ...
        substitute_page_image: allImgs.filter(i => vis(i)).map(i => (i.currentSrc || i.src)).filter(s => !s.includes('avatars.githubusercontent.com'))
      } };
  }
...
async function run(viewportId, motion, checkpoints) {
  ...
  rows.push({ key: `__context__/${viewportId}/${motion}`, _console_errors: [...new Set(errs)], _failed_requests: [...new Set(failed)] });
  await ctx.close();
}
