...
async function capture(tag, { viewport, motion, locale, route }) {
  ...
  if (!ready.assetsReady) throw new Error(`${tag}: assets not ready`);
  ...
  if (!last?.settle?.settled) throw new Error(`${tag}: did not settle`);
  ...
  return { path: p, sha: crypto.createHash('sha256').update(await fs.readFile(p)).digest('hex'),
    documentProgress: last.documentProgress ?? null, score: last.score ?? null, errors: [...new Set(errors)] };
}
